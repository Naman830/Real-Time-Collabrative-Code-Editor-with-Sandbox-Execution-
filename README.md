...
Everything above is **single-instance**: awareness is broadcast only to sockets connected to the same server process. Extending live cursors across instances (so presence converges the same way document edits will) is scaffolded — but not yet implemented — in `server/redis/awareness.js`; see [Cross-Instance Sync](#cross-instance-sync-scaffold).
...
- **`redis/channels.js`** — the channel naming scheme and message envelopes, one pair per relay: `syncChannel(roomId)` → `room:<roomId>:sync` with the `SyncEnvelope` shape (`{ roomId, update, originInstanceId }`) for document updates, and the parallel `awarenessChannel(roomId)` → `room:<roomId>:awareness` with a structurally identical `AwarenessEnvelope` for presence. Same `{ roomId, update, originInstanceId }` shape for both — the only difference is that awareness's `update` is an `encodeAwarenessUpdate` payload rather than a `Y.Doc` update. One channel per room per relay, so an instance only receives traffic for rooms it actually hosts.
...
- **`redis/sync.js`** — `startRoomSync(roomId, ydoc)` / `subscribeRoom(roomId, ydoc)` / `stopRoomSync(roomId, ydoc)`.
  - **Publish half (`startRoomSync`)** attaches a **second, independent `"update"` listener** to the room's `Y.Doc` — separate from (and not chained off) the debounced-snapshot listener described in [Persistence](#persistence) above, guarded the same way against duplicate attachment across multiple connections to the same room. `stopRoomSync` detaches it. `server/yjsConnection.js`'s `handleYjsConnection` calls `startRoomSync` when a room starts being hosted and `stopRoomSync` on that room's last disconnect, alongside (not touching) the existing persistence hooks. Inside the listener sits `// TODO(core-logic): publish this update + this instance's ID to Redis on the room's channel` — the actual `publisher.publish(...)` call and echo-loop prevention (telling "update that originated here" apart from "update that just arrived from Redis") are still open.
  - **Subscribe half (`subscribeRoom`)** is the newer scaffold: it defines a **stub message handler** for a room's channel carrying the `// TODO(core-logic): check message.instanceId against local instance ID, skip if self-origin, otherwise Y.applyUpdate` comment (no `Y.applyUpdate` is called yet), plus a second TODO for the subscribe *mechanics* (which channel, dispatch, once-per-room). Critically, `subscribeRoom` is **exported but deliberately not called anywhere yet** — the *timing* of the subscribe relative to the Neon snapshot load and the client's initial sync is an ordering-guarantee decision left open, marked by a `// TODO(core-logic): decide exactly where this subscribe call goes relative to snapshot-load-from-Neon and client-sync…` comment sitting in `handleYjsConnection` between those two boundaries.
- **`redis/awareness.js`** — the presence relay, deliberately a structural mirror of `redis/sync.js`: `startRoomAwarenessSync(roomId, ydoc)` / `subscribeRoomAwareness(roomId, ydoc)` / `stopRoomAwarenessSync(roomId, ydoc)`, same start/subscribe/stop shape and same instance-ID + payload envelope.
  - The difference is *what* it relays: it attaches its second, independent `"update"` listener to `ydoc.awareness` (the `y-protocols/awareness` `Awareness` instance `y-websocket`'s `WSSharedDoc` creates and broadcasts locally) rather than to the `Y.Doc`, and the payload is an `encodeAwarenessUpdate` applied with `applyAwarenessUpdate` on the far side, not `Y.applyUpdate`. The local single-process awareness broadcast (from [Presence](#presence-multi-cursor-awareness)) is left completely untouched — this only fans the same change out to *other* instances. `handleYjsConnection` calls `startRoomAwarenessSync` alongside `startRoomSync`, and `stopRoomAwarenessSync` alongside `stopRoomSync` on last disconnect; `subscribeRoomAwareness` is exported-but-unwired for the same timing reason as `subscribeRoom`. Publish and self-origin/apply carry the same `// TODO(core-logic)` markers as the doc relay.
  - **Presence-specific open decision:** because awareness state is ephemeral and applied remote states linger until something removes them, a remote instance that *crashes* (rather than disconnecting cleanly) never publishes the "removed" update that would clear its clients' cursors — leaving frozen ghost cursors. How to reap those is left explicitly open with `// TODO(core-logic): decide how to handle stale cursors when a remote instance dies without a clean disconnect (TTL? heartbeat? piggyback on existing awareness timeout?)` in `subscribeRoomAwareness`. It sits on the *apply* side, not in `stopRoomAwarenessSync`, precisely because that teardown only fires when *this* instance's last client leaves — never when some *other* instance dies, which is the case that strands the cursors.
